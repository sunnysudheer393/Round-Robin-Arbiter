...
//request gets eventually deasserted
assume property ( @(posedge clk) disable iff(rst) req[symb_req] |-> s_eventually !req[symb_req]);
...
// req stays high until it is granted
assume property (@(posedge clk) disable iff(rst)
    req[symb_req1] && !gnt[symb_req1] |=> req[symb_req1]);

assume property (@(posedge clk) disable iff(rst)
    req[symb_req2] && !gnt[symb_req2] |=> req[symb_req2]);
...
always_ff @( posedge clk) begin
    if(rst) begin
        ...
        if (gnt[symb_req2]) req_to_be_gntd <= 1'b0;
        else if( req[symb_req1] && req[symb_req2] && gnt[symb_req1]) req_to_be_gntd <= 1'b1;
    end
end

assert property ( @(posedge clk) disable iff(rst) gnt[symb_req1] |-> !req_to_be_gntd);

assert property ( @(posedge clk) disable iff(rst) req[symb_req1] && req[symb_req2] && gnt[symb_req1] |=> !gnt[symb_req1] until_with gnt[symb_req2]);

//assert property ( @(posedge clk) disable iff(rst) req[symb_req1] && req[symb_req2] && gnt[symb_req1] |=> !gnt[symb_req1] );

assert property ( @(posedge clk) disable iff(rst) (|req) |-> (|gnt) );
//assert property ( @(posedge clk) disable iff(rst) (symb_req1 < symb_req2) && req[symb_req1] && req[symb_req2] && !gnt[symb_req1] |-> !gnt[symb_req2]);
